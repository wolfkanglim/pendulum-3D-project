...
function initThree(){
     ...
     1, 500)
     cameraFront.position.set(0, 20, 70);
...
function cameraMove(){
     ...
     tl.to(camera.positoin, {y: 10 * 0 , duration: 6});
     ...
     tl.to(camera.position, {s: 0, duration:4});  
...
function createBar(){
     ...
     bar.position.set(0, 32, 0);
     bar.scale.set(1, 1, 54);
     scene.add(bar);

     const beam = createCube();
     beam.position.set(0, 0, -18);
     beam.scale.set(2, 65, 2);
     scene.add(beam);
...
function createVibraphone(){
     ...
     for(let i = 0; i < 21; i++){
          ...
          cube.material.color = new THREE.Color(colors[i]);
          cube.scale.set(1.5, i * 0.5 + 5, 0.5);
          cube.position.set(-6 - (i * 2.02), 16 + (i * 0.5), i * 0.18);
          leftCubes.push(cube);
          groupLeft.add(cube);
...
for(let i = 0; i < maxPendulum; i++){
     const pendulum = createPendulum(new THREE.Vector3(0, 0, -i * 2 + 24), 1 + i * 0.0125, 1 + i * 0.0125);
          ...
          pendulums.forEach((p, index) => {
               ...
               if(p.ball.rotation.z >= (0.999 + index * 0.01245)) {
                    ...
                    if(pulseEnabled){
                         ...
                         gsap.to(rightCubes[index].position, {y: index/2 + 16, duration: 2}) ;
                    ...
                    if(pulseEnabled){
                         ...
                         gsap.to(leftCubes[index].position, {y: 15 + (index * 1.0125)/2 + 0.9, duration: 2}) ;  
